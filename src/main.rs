...
async fn main() -> anyhow::Result<()> {
    ...
    tracing::info!("Starting Shade Identity Provider on {}:{}", config.server.host, config.server.port);
    ...
    let auth_service = services::auth::AuthService::new(db_pool.clone(), session_service.clone());
...
async fn create_app(
    ...
    Router::new()
        ...
        .route("/login", get(auth_handlers::login_page).post(auth_handlers::login))
        ...
        .route("/api/admin/*path", get(admin::api_handler).post(admin::api_handler))
